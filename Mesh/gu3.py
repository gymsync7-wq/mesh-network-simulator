...
def fail_node_on_path(G, path):
    """Fail a random intermediate node on the active path."""
...
def fail_multiple_nodes(G, path, count=2):
    """Fail multiple random intermediate nodes on the active path."""
    candidates = [n for n in path[1:-1] if not G.nodes[n]['failed']]
    failed = []
...
class MeshNetworkApp(tk.Tk):
    ...
    def _fail_node(self):
        ...
        self._draw()
        self._log(f"Node {node} FAILED on path!", "error")
        self._log(f"Broken path: {' → '.join(map(str,self.current_path))}", "warn")
        self._log(f"Click 'Reroute Node' — Dijkstra will find shortest alternate path.", "info")
        self._status(f"⚠ Node {node} failed — click Reroute Node for Dijkstra shortest path")
    ...
    def _fail_multiple(self):
        ...
        self._log(f"Click 'Reroute Node' — Dijkstra will find shortest alternate path.", "info")
        self._status(f"⚠ Nodes {failed} failed — click Reroute Node for Dijkstra shortest path")
    ...
    def _reroute(self):
        ...
        if new_path:
            self.current_path = new_path
            self._draw()
            self._log("REROUTING via Dijkstra Shortest Path Algorithm...", "warn")
            self._log(f"Old path  : {' → '.join(map(str,old_path))}", "warn")
            self._log(f"New path  : {' → '.join(map(str,new_path))} ({new_cost} units)", "success")
            self._log(f"Hops: {len(new_path)-1}  |  Distance: {new_cost} units  |  Algorithm: Dijkstra", "info")
            self._status(f"✔ Dijkstra Reroute → {' → '.join(map(str,new_path))} ({new_cost} units)")
            self._add_history(old_path, new_path, failed_nodes)
        else:
            self._log("Dijkstra found NO alternate path — network partitioned!", "error")
            self._status("✘ No alternate path — network partitioned!")
